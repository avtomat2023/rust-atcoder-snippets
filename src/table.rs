...
impl<T> Table<T> {
    ...
    /// ]);
    /// ```
    pub fn indices(&self) -> TableIndicesIterator {
    ...
    pub fn adjacent8_indices(&self, (y, x): (usize, usize)) -> Option<Vec<(usize, usize)>> {
        self.inside((y, x)).then_with(|| {
            ...
            let xs = x.saturating_sub(1) .. std::cmp::min(x+1, self.width()-1)+1;
...
impl<T, F1: Fn(&T, &T) -> T, F2: Fn(&T, &T) -> T> CumulativeTable<T, F1, F2> {
    ...
    pub fn query_yx(&self, y: std::ops::Range<usize>, x: std::ops::Range<usize>) -> Option<T> {
